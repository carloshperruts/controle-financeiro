...
async def main(page: ft.Page):
    ...
    confirmar_senha_input = ft.TextField(label="Confirmar Senha", password=True, can_reveal_password=True, width=320, visible=False)

    dica_cadastro = ft.Container(
        content=ft.Column([
            ft.Text("💡 Dicas para criar sua conta:", size=12, weight=ft.FontWeight.BOLD, color="#00FF66"),
            ft.Text("• Digite um e-mail válido para receber a confirmação.", size=11, color=ft.Colors.GREY_300),
            ft.Text("• A senha deve ter no mínimo 6 caracteres.", size=11, color=ft.Colors.GREY_300),
        ], spacing=3),
        padding=10,
    ...
    def realizar_login(e):
        ...
        if not email_input.value or not senha_input.value:
            msg_erro.value = "⚠️ Preencha e-mail e senha para entrar."
            page.update()
            return
    ...
    def realizar_cadastro(e):
        ...
        conf_senha = confirmar_senha_input.value

        if not email or "@" not in email or "." not in email:
            msg_erro.value = "⚠️ Digite um e-mail válido (ex: seunome@email.com)."
            page.update()
            return

        if len(senha) < 6:
            msg_erro.value = "⚠️ A senha deve conter pelo menos 6 caracteres."
            page.update()
            return

        if senha != conf_senha:
            msg_erro.value = "⚠️ As senhas digitadas não coincidem."
            page.update()
            return
        ...
        try:
            ...
            mostrar_notificacao("✔ Conta criada com sucesso! Verifique seu e-mail para confirmar.")
            mudar_para_login(None)
        except Exception as ex:
    ...
    def recuperar_senha(e):
        ...
        if not email or "@" not in email or "." not in email:
            msg_erro.value = "⚠️ Digite seu e-mail no campo acima para solicitar a recuperação."
            page.update()
            return

        try:
            supabase.auth.reset_password_for_email(email)
            mostrar_notificacao("✉️ Link de redefinição de senha enviado para o seu e-mail!")
        except Exception as ex:
            msg_erro.value = f"❌ Erro ao solicitar recuperação: {str(ex)}"
            page.update()

    # --- NAVEGAÇÃO CLARA ENTRE LOGIN E CADASTRO ---
    def mudar_para_cadastro(e):
        msg_erro.value = ""
        confirmar_senha_input.visible = True
        dica_cadastro.visible = True
        btn_entrar.visible = False
        btn_cadastrar.visible = True
        btn_esqueci_senha.visible = False
        btn_ir_cadastro.visible = False
        btn_voltar_login.visible = True
        page.update()

    def mudar_para_login(e):
        msg_erro.value = ""
        confirmar_senha_input.visible = False
        dica_cadastro.visible = False
        btn_entrar.visible = True
        btn_cadastrar.visible = False
        btn_esqueci_senha.visible = True
        btn_ir_cadastro.visible = True
        btn_voltar_login.visible = False
        page.update()

    btn_entrar = ft.ElevatedButton("Entrar", on_click=realizar_login, bgcolor="#00AA44", color=ft.Colors.WHITE, width=320)
    btn_cadastrar = ft.ElevatedButton("Finalizar Cadastro", on_click=realizar_cadastro, bgcolor="#00AA44", color=ft.Colors.WHITE, width=320, visible=False)
    
    btn_esqueci_senha = ft.TextButton("Esqueceu a senha?", on_click=recuperar_senha, style=ft.ButtonStyle(color=ft.Colors.GREY_400))
    btn_ir_cadastro = ft.TextButton("Primeiro acesso? Criar nova conta", on_click=mudar_para_cadastro, style=ft.ButtonStyle(color="#00FF66"))
    btn_voltar_login = ft.TextButton("← Voltar para a tela de Login", on_click=mudar_para_login, style=ft.ButtonStyle(color="#00FF66"), visible=False)
    ...
    def carregar_tela_principal():
        try:
            ...
            def carregar_renda_base():
                if not user_id:
                    return
                try:
                    res = supabase.table("profiles").select("renda_base").eq("id", user_id).execute()
                    if res.data and len(res.data) > 0:
                        val = float(res.data[0].get("renda_base", 0.0))
                        txt_renda_base.value = f"{val:.2f}".replace(".", ",")
                    else:
                        txt_renda_base.value = "0,00"
                except Exception:
                    txt_renda_base.value = "0,00"

            def salvar_renda_base(e=None):
                if not user_id:
                    return
                try:
                    val_num = float(txt_renda_base.value.replace(".", "").replace(",", "."))
                    supabase.table("profiles").upsert({"id": user_id, "renda_base": val_num}).execute()
                    mostrar_notificacao("✔ Renda base atualizada!")
                    carregar_registros()
                except ValueError:
                    mostrar_notificacao("Informe um valor numérico válido para a renda!", ft.Colors.RED_600)
                except Exception as ex:
                    mostrar_notificacao(f"Erro ao salvar renda: {str(ex)}", ft.Colors.RED_600)
            ...
            def calcular_totais(registros):
                ...
                lbl_gastos.value = f"Despesas: R$ {tot_gasto:.2f}"
                ...
                dialog = ft.AlertDialog(
                    ...
                    content=ft.Column(
                        [
                            ...
                            ft.Row([
                                ft.Text(f"Total Despesas: R$ {tot_gastos:.2f}", color=ft.Colors.RED_400),
            ...
            conteudo_principal = ft.Column(
                [
                    ...
                    ft.Row([txt_renda_base, ft.ElevatedButton("Atualizar", on_click=salvar_renda_base)]),
